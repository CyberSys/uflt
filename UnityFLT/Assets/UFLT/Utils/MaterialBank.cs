    ...
    public class MaterialBank
    {
        ...
        public Texture2D FindOrCreateTexture(TexturePalette tp)
        {
            ...
            if (path != string.Empty)
            {
                ...
                var textureFromAssets = UnityEditor.AssetDatabase.LoadAssetAtPath<Texture2D>("Assets" + cleanPath.Replace(Application.dataPath, "")) as Texture2D;
                Debug.Log(path);
                if (textureFromAssets != null)
                {
                    _Textures[path] = textureFromAssets;
                    return textureFromAssets;
                }
                #endif
